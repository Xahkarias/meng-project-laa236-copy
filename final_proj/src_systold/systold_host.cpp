...
int main(int argc, char* argv[]) {
    ...
    if (mode == 'F') {
        xclbin = xrt::xclbin("build_dir.hw.xilinx_u250_gen3x16_xdma_4_1_202210_1/SW_systold.link.xclbin");
        std::cout << "Running in FPGA mode: " << std::endl;
        myDevice = xrt::device("0000:88:00.1");
        
    } else if (mode == 'S') {
        xclbin = xrt::xclbin("SW_systold_sw.xclbin");
        std::cout << "Running in sw emu mode" << std::endl;
        myDevice = xrt::device(0);
        
    } else if (mode == 'H') {
        xclbin = xrt::xclbin("SW_systold_hw.xclbin");
